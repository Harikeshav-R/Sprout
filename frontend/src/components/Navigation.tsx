...
export default function Navigation() {
  ...
  const hideNav = location.pathname === '/dashboard' || location.pathname === '/discovery';
  ...
  const showSolidBg = isScrolled || isAppPage || isOnboarding;
  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          {(isAppPage || isOnboarding) ? (
            ...
            >
              <div className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${showSolidBg ? 'bg-sprout-green' : 'bg-white/20 backdrop-blur-sm'
                }`}>
                <Sprout className={`w-5 h-5 ${showSolidBg ? 'text-sprout-gold' : 'text-white'}`} />
              </div>
              <span className={`font-display font-bold text-lg tracking-tight transition-colors ${showSolidBg ? 'text-sprout-green' : 'text-white'
                }`}>
                Sprout
              </span>
            </a>
          ...
          {/* Desktop Navigation - hidden on onboarding */}
          {!isOnboarding && (
              ...
              {isAppPage ? (
                appNavLinks.map((link) => (
                  <Link
                    key={link.label}
                    to={link.href}
                    className={`text-sm font-medium transition-colors hover:text-sprout-gold ${showSolidBg ? 'text-gray-700' : 'text-white/90'
          ...
          {/* CTA Button - hidden on onboarding */}
          {!isOnboarding && (
          <div className="hidden md:block">
            {isAppPage ? (
              <Link
                to="/dashboard"
                className={`px-5 py-2.5 rounded-full font-semibold text-sm transition-all duration-300 ${showSolidBg
          ...
          {/* Mobile Menu Button - hidden on onboarding */}
          {!isOnboarding && (
            <button
      ...
      {/* Mobile Menu - hidden on onboarding */}
      {!isOnboarding && (
      <div
        className={`fixed inset-0 z-[99] bg-sprout-green transition-all duration-500 md:hidden ${isMobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'
          }`}
      >
